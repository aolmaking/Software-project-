...
def view_cart():
    ...
    return _json_with_session(_cart_response(session_id), session_id), 200
...
def add_to_cart():
    ...
    if not isinstance(payload, dict):
        return jsonify({"error": "Request body must be valid JSON", "code": "BAD_REQUEST"}), 400

    item_public_id = str(payload.get("item_id", "")).strip()

    valid, error_message, quantity = _parse_quantity(payload.get("quantity", 1))
    if not valid:
        return jsonify({"error": error_message, "code": "INVALID_QUANTITY"}), 422
    ...
    if not bool(item["available"]):
        return jsonify({"error": "Item is currently unavailable", "code": "ITEM_UNAVAILABLE"}), 409
    ...
    if existing:
        db.execute(
            ...
            WHERE session_id = ? AND item_public_id = ?
            """,
            (new_quantity, session_id, item_public_id),
        )
    else:
        db.execute(
            """
